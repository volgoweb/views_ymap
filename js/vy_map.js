...
(function ($) {
  ...
  Drupal.views_ymap.config = {
    base_zoom: 15,
    id_map_container: 'container-ymap', 
  };
  ...
  Drupal.views_ymap.show_objects = function(objects) {
    ...
    for (i in objects) {
      var placemark = new ymaps.Placemark([objects[i].lat,objects[i].lon], {
          iconContent: number,
          balloonContentHeader: '<div class="balloon__header">'+objects[i].balloon.title+'</div>',
          balloonContentBody: '<div class="balloon__body">'+objects[i].balloon.body + '</div>',
          nid: objects[i].nid,   
        }, {
          // Опции
  ...
  Drupal.behaviors.vy_map = {
    attach: function (context, settings) {
      ...
      ymaps.ready(function () {
        Drupal.views_ymap.map = new ymaps.Map(config.id_map_container, {
